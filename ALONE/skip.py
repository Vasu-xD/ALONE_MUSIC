...
from ALONE.handlers import skip_current_song, skip_item
from ALONE.queues import QUEUE, clear_queue
...
async def skip(client, m: Message):
   ...
   if len(m.command) < 2:
      ...
      if op==0:
         ...
         await m.reply(f"**Some Error Occurred** \n`Clearing the Queues and ₳ⱠØ₦Ɇ Is Leaving the Voice Chat...`")
      else:
         await m.reply(f"**Skipped ⏭** \n**🎧 Now ₳ⱠØ₦Ɇ Is Playing** - [{op[0]}]({op[1]}) | `{op[2]}`", disable_web_page_preview=True)
   else:
